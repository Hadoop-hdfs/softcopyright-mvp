...
def set_font(run, font_name: str | None = None, font_size: float | None = None, bold: bool = False):
    """设置字体（中文必须用中文字体名）"""
    selected_font_name = font_name or config.DOC_BODY_FONT
    selected_font_size = font_size or config.DOC_BODY_FONT_SIZE
    run.font.name = selected_font_name
...
def add_page_header_footer(
    doc,
    software_name: str,
    total_pages: int = config.SOURCE_FRONT_PAGES + config.SOURCE_BACK_PAGES,
):
    """设置页眉页脚"""
    ...
    set_font(run, config.DOC_HEADER_FONT, config.DOC_HEADER_FONT_SIZE)
    ...
    set_font(run, config.DOC_HEADER_FONT, config.DOC_FOOTER_FONT_SIZE)
...
def create_source_code_doc(code_dir: str, output_path: str, software_name: str):
    ...
    style.font.name = config.DOC_BODY_FONT
    style.font.size = Pt(config.DOC_SOURCE_BODY_FONT_SIZE)  # 五号字
    style.paragraph_format.line_spacing = 1.5
    style._element.rPr.rFonts.set(qn('w:eastAsia'), config.DOC_BODY_FONT)

    # 标题
    title = doc.add_paragraph()
    title.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = title.add_run(f"{software_name} - 源代码")
    set_font(run, config.DOC_TITLE_FONT, config.DOC_SOURCE_TITLE_FONT_SIZE, bold=True)
    ...
    if total_lines <= (FRONT_PAGES + BACK_PAGES) * LINES_PER_PAGE:
        ...
        for rel_path, lines in all_lines:
            ...
            set_font(run, config.DOC_TITLE_FONT, 11, bold=True)

            for line in lines:
                para = doc.add_paragraph()
                run = para.add_run(line.rstrip())
                set_font(run, config.DOC_CODE_FONT, config.DOC_CODE_FONT_SIZE)
    else:
        # 截取前 FRONT_PAGES 页
        line_count = 0
    ...
    max_pages = FRONT_PAGES + BACK_PAGES
    add_page_header_footer(doc, software_name, total_pages=min(max_pages, total_lines // LINES_PER_PAGE + 1))
    doc.save(output_path)
...
def create_description_doc(p1_data: dict, p2_data: dict, screenshot_dir: str, output_path: str, software_name: str):
    ...
    style = doc.styles['Normal']
    style.font.name = config.DOC_BODY_FONT
    style.font.size = Pt(config.DOC_BODY_FONT_SIZE)
    style.paragraph_format.line_spacing = 1.5
    style._element.rPr.rFonts.set(qn('w:eastAsia'), config.DOC_BODY_FONT)
    ...
    set_font(run, config.DOC_TITLE_FONT, config.DOC_TITLE_FONT_SIZE, bold=True)
    ...
    if screenshots:
        for i, ss_path in enumerate(screenshots, 1):
            ...
            try:
                doc.add_picture(ss_path, width=Cm(config.SCREENSHOT_WIDTH_CM))
